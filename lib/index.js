		...
		if ( config.deployment.toLocal.enabled === true && config.deployment.toLocal.pathFetching === true ) {
			senseLoc.getLocalExtensionPath( function ( err, data ) {
				...
				if ( !_.isEmpty( data ) ) {
					...
					var extName = (!_.isEmpty( config.deployment.toLocal.extensionDirOverride ) ? sanitize( config.deployment.toLocal.extensionDirOverride ) : sanitize( config.packageName ));
					config.deployment.toLocal.extensionPath = expandTile( path.join( config.deployment.toLocal.localExtensionsBaseDir, extName ) );
				}
				cb( err );
			} );
