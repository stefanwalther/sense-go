...
var senseGo = function () {
	...
	function init ( gulpObj, userConfig, callback ) {
		...
		var senseGoFile = path.join( process.cwd(), '.sense-go.yml' );
		if ( userConfig === null && !fs.existsSync( senseGoFile ) ) {
			gUtil.log( gUtil.colors.red( 'Missing either a custom configuration or a .sense-go.yml file' ) );
			return;
		}
		if ( userConfig === null && fs.existsSync( senseGoFile ) ) {
			userConfig = yaml.safeLoad( fs.readFileSync( senseGoFile , 'utf8' ) );
		}
