...
function CleanTasks ( gulp, plugins, config, taskUtils ) {
	 ...
	 * @param delOptions
	 */
	var clean = function( taskConfig, delOptions, done ) {
		return del( taskConfig.src, delOptions ).then( function ( paths ) {
			// paths.forEach( function( path) {
			// 	taskUtils.log( taskConfig.taskName, gUtil.colors.blue( path ));
			// });
			// taskUtils.log( taskConfig.taskName, gUtil.colors.green( paths.length + ' items' ) );
			done();
		} );
	};
