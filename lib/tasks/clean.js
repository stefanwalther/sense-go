...
function CleanTasks ( gulp, plugins, config, taskUtils ) {
	...
	function clean ( taskConfig, delOptions, done ) {
		return del( taskConfig.src, delOptions, function ( err, paths ) {
			//taskUtils.log( 'del paths', paths ); //Todo: Refactor or remove
	...
	gulp.task( 'clean:tmp', function ( done ) {
		return clean( {taskName: 'clean:tmp', src: path.resolve( config.tmpDir )}, {}, done );
	} );

	gulp.task( 'clean:buildDev', function ( done ) {
		return clean( {taskName: 'clean:buildDev', src: path.resolve( config.buildDevDir )}, {}, done );
	} );

	gulp.task( 'clean:buildRelease', function ( done ) {
		return clean( {taskName: 'clean:buildRelease', src: path.resolve( config.buildReleaseDir )}, {}, done );
	} );
