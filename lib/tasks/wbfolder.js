	...
	function createWbFolderFile ( taskConfig, done ) {
		...
		if ( config.wbfolder.enabled === true ) {
			glob( taskConfig.src, {cwd: taskConfig.cwd}, function ( err, files ) {
				...
				fs.writeFile( path.join( taskConfig.cwd, taskConfig.dest ), wbFolderContent.join( lineSep ), function ( err ) {
					if ( err ) {
						return console.error( 'An error occurred while saving the wbfolder.wbl file', err ); //Todo: better error-handling & -output
					}
					return done();
				} );
			} )
