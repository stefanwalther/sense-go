...
function SshTasks(gulp, plugins, config /* , taskUtils */) {
  ...
  gulp.task('deploy:toSsh', function () {
    ...
    if (config.deployment.toSsh.enabled) {
      ...
      gUtil.log(gUtil.colors.gray('ssh.user: ', config.deployment.toSsh.user));

      var gulpSsh = new GulpSshInst(config.deployment.toSsh);
    ...
    gUtil.log(gUtil.colors.gray('Task "deploy:toSsh" is disabled'));
  ...
  gulp.task('import:fromSsh', function () {
    ...
    if (config.import.fromSsh.enabled) {
      var gulpSsh = new GulpSshInst(config.import.fromSsh);
    ...
    gUtil.log(gUtil.colors.gray('Task "import:fromSsh" is disabled'));
    return gulp.src('.')
      .pipe(plugins.nop());
