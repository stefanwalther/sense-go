  ...
  gulp.task('deploy:viaShell', function (cbMain) {
    ...
    if (config.deployment.viaShell && config.deployment.viaShell.enabled && (config.deployment.viaShell.cmd || config.deployment.viaShell.commands)) {
      ...
      if (config.deployment.viaShell.cmd) {
        console.log('legacy stuff');
        commands.push(
          function (cb) {
            gUtil.log('shell', gUtil.colors.blue(_.truncate(config.deployment.viaShell.cmd, {length: 75})));
        ...
        gUtil.log('deploy:viaShell', gUtil.colors.red('Tasks on the root of deployment.viaShell are deprecated, move it to \"commands\"'));
      }
      ...
      if (config.deployment.viaShell.commands) {
        config.deployment.viaShell.commands.forEach( command => {
          if (command.enabled || command.enabled === undefined) {
            commands.push(
              function (cb) {
                gUtil.log('shell', gUtil.colors.blue(_.truncate(command.cmd, {length: 75})));
